...
export default function PongGame() {
  ...
  const loop = useCallback(() => {
    ...
    if (s.ball.y + BALL_SIZE / 2 >= H) {
      s.ball.y = H - BALL_SIZE / 2;
      s.ball.vy = -Math.abs(s.ball.vy);
    }
    ...
    if (speed > 12) {
      s.ball.vx = (s.ball.vx / speed) * 12;
      s.ball.vy = (s.ball.vy / speed) * 12;
    }
    ...
    <div className="flex flex-col items-center gap-4 p-4 select-none">
      <div className="flex items-center gap-12 font-orbitron text-sm">
        <div className="flex items-center gap-2">
          <span className="text-foreground/50">P1</span>
          <span className="neon-text-cyan font-bold text-2xl">{p1Score}</span>
        </div>
        <div className="text-foreground/30 text-xs">FIRST TO {WIN_SCORE}</div>
        <div className="flex items-center gap-2">
          <span className="neon-text-cyan font-bold text-2xl">{p2Score}</span>
          <span className="text-foreground/50">P2</span>
        </div>
      </div>
      <div className="relative">
        ...
        {(!started || gameOver) && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 rounded gap-4">
            {gameOver && (
              <div className="font-arcade text-lg neon-text-yellow">
                PLAYER {winner} WINS!
              </div>
            )}
            {!started && !gameOver && (
