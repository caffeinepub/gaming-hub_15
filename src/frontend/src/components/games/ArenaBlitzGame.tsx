...
export default function ArenaBlitzGame() {
  ...
  const spawnRound = useCallback((round: number) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const type =
        round > 2 && Math.random() < 0.3
          ? "shooter"
          : round > 1 && Math.random() < 0.3
            ? "bouncer"
            : "chaser";
      s.enemies.push({
        id: enemyId++,
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (
        e.code === "Enter" &&
        (!stateRef.current.running || stateRef.current.gameOver)
      )
        startGame();
    };
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: canvasRef2.current is a stable ref
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.strokeRect(
        ARENA_PADDING,
        ARENA_PADDING,
        CANVAS_W - ARENA_PADDING * 2,
        CANVAS_H - ARENA_PADDING * 2,
      );
      ctx.shadowBlur = 0;
      ...
      ctx.lineWidth = 1;
      for (let x = ARENA_PADDING; x < CANVAS_W - ARENA_PADDING; x += 40) {
        ctx.beginPath();
        ctx.moveTo(x, ARENA_PADDING);
        ctx.lineTo(x, CANVAS_H - ARENA_PADDING);
        ctx.stroke();
      }
      for (let y = ARENA_PADDING; y < CANVAS_H - ARENA_PADDING; y += 40) {
        ctx.beginPath();
        ctx.moveTo(ARENA_PADDING, y);
        ctx.lineTo(CANVAS_W - ARENA_PADDING, y);
        ctx.stroke();
      }
      ...
      const p = s.player;
      const alpha =
        p.iframes > 0 ? (Math.floor(p.iframes / 4) % 2 === 0 ? 0.3 : 1) : 1;
    ...
    const update = () => {
      ...
      if (s.keys.has("ArrowLeft") || s.keys.has("KeyA"))
        p.x = Math.max(ARENA_PADDING + PLAYER_R, p.x - PLAYER_SPEED);
      if (s.keys.has("ArrowRight") || s.keys.has("KeyD"))
        p.x = Math.min(CANVAS_W - ARENA_PADDING - PLAYER_R, p.x + PLAYER_SPEED);
      if (s.keys.has("ArrowUp") || s.keys.has("KeyW"))
        p.y = Math.max(ARENA_PADDING + PLAYER_R, p.y - PLAYER_SPEED);
      if (s.keys.has("ArrowDown") || s.keys.has("KeyS"))
        p.y = Math.min(CANVAS_H - ARENA_PADDING - PLAYER_R, p.y + PLAYER_SPEED);
      ...
      for (const e of s.enemies) {
        if (e.type === "chaser") {
          ...
          if (e.x < ARENA_PADDING + e.r || e.x > CANVAS_W - ARENA_PADDING - e.r)
            e.vx *= -1;
          if (e.y < ARENA_PADDING + e.r || e.y > CANVAS_H - ARENA_PADDING - e.r)
            e.vy *= -1;
        ...
        e.x = Math.max(
          ARENA_PADDING + e.r,
          Math.min(CANVAS_W - ARENA_PADDING - e.r, e.x),
        );
        e.y = Math.max(
          ARENA_PADDING + e.r,
          Math.min(CANVAS_H - ARENA_PADDING - e.r, e.y),
        );
      }
      ...
      s.bullets = s.bullets.filter(
        (b) => b.x > 0 && b.x < CANVAS_W && b.y > 0 && b.y < CANVAS_H,
      );
      ...
      if (p.iframes <= 0) {
        ...
        for (const b of s.bullets) {
          if (
            !b.fromPlayer &&
            Math.hypot(b.x - p.x, b.y - p.y) < PLAYER_R + 5
          ) {
            p.hp--;
            p.iframes = 60;
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
      <div className="relative">
        ...
        {gameState === "idle" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/75 rounded-lg">
            <h2
              className="font-arcade text-3xl text-orange-400 mb-2"
              style={{ textShadow: "0 0 20px #fb923c" }}
            >
              ARENA BLITZ
            </h2>
            <p className="font-rajdhani text-orange-300 text-lg mb-6">
              Survive the arena
            </p>
            <button
              type="button"
              onClick={startGame}
              className="font-arcade text-sm bg-orange-700 hover:bg-orange-600 text-white px-6 py-3 rounded border border-orange-400 transition-all"
            >
              START GAME
            </button>
        ...
        {gameState === "over" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
            <h2
              className="font-arcade text-2xl text-red-400 mb-2"
              style={{ textShadow: "0 0 20px #f43f5e" }}
            >
              DEFEATED!
            </h2>
            <p className="font-arcade text-orange-400 text-sm mb-1">
              SCORE: {displayScore}
            </p>
            <p className="font-arcade text-yellow-400 text-sm mb-1">
              ROUND: {displayRound}
            </p>
            <p className="font-arcade text-white/50 text-xs mb-6">
              BEST: {stateRef.current.highScore}
            </p>
            <button
              type="button"
              onClick={startGame}
      ...
      <p className="font-rajdhani text-orange-400/60 text-sm">
        WASD to move • Click to shoot toward cursor
      </p>
    </div>
  );
}
