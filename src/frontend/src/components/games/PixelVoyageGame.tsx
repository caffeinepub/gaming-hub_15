...
export default function PixelVoyageGame() {
  ...
  const startGame = useCallback(() => {
    ...
    s.pipes = [
      {
        x: CANVAS_W + 100,
        topH: 100 + Math.random() * (CANVAS_H - PIPE_GAP - 200),
        scored: false,
      },
    ];
    s.particles = [];
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (
        s.pipes.length === 0 ||
        s.pipes[s.pipes.length - 1].x < CANVAS_W - 220
      ) {
        s.pipes.push({
          x: CANVAS_W,
          topH: 80 + Math.random() * (CANVAS_H - PIPE_GAP - 160),
          scored: false,
        });
      }
      ...
      for (const pipe of s.pipes) {
        if (
          BIRD_X + birdR > pipe.x + 4 &&
          BIRD_X - birdR < pipe.x + PIPE_W - 4
        ) {
          if (
            s.bird.y - birdR < pipe.topH ||
            s.bird.y + birdR > pipe.topH + PIPE_GAP
          ) {
            s.gameOver = true;
            s.running = false;
            if (s.score > s.highScore) s.highScore = s.score;
      ...
      <div className="relative">
        ...
        {gameState === "over" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
            ...
            <p className="font-arcade text-green-400 text-xl mb-1">
              {displayScore}
            </p>
