...
export default function VoxelRealmsGame() {
  ...
  >("idle");
  const [_displayGold, setDisplayGold] = useState(100);
  const [_displayLives, setDisplayLives] = useState(20);
    ...
    const update = () => {
      ...
      for (const tower of s.towers) {
        ...
        for (const e of s.enemies) {
          ...
          if (d < tower.range * CELL_W && d < closestDist) {
            closestDist = d;
            closest = e;
          }
        }
  ...
  return (
    <div className="flex flex-col items-center gap-3 select-none">
      <div className="relative">
        ...
        {gameState === "idle" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/75 rounded-lg">
            <h2
              className="font-arcade text-3xl text-green-400 mb-2"
              style={{ textShadow: "0 0 20px #4ade80" }}
            >
              VOXEL REALMS
            </h2>
            <p className="font-rajdhani text-green-300 text-lg mb-6">
              Tower defense — protect your base!
            </p>
            <button
              type="button"
              onClick={startGame}
              className="font-arcade text-sm bg-green-700 hover:bg-green-600 text-white px-6 py-3 rounded border border-green-400 transition-all"
            >
              START GAME
            </button>
      ...
      <div className="flex gap-3">
        <button
          ...
          onClick={() => {
            ...
            stateRef.current.selectedTool = "tower";
          }}
        ...
        <button
          ...
          onClick={() => {
            ...
            stateRef.current.selectedTool = "wall";
          }}
      ...
      <p className="font-rajdhani text-green-400/60 text-sm">
        Click empty tiles to build • Enemies follow the green path
      </p>
    </div>
  );
}
