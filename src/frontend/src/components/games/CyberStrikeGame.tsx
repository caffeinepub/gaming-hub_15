...
export default function CyberStrikeGame() {
  ...
  const spawnWave = useCallback((wave: number) => {
    ...
    for (let i = 0; i < count; i++) {
      const type =
        wave > 3 && Math.random() < 0.2
          ? "tank"
          : wave > 1 && Math.random() < 0.3
            ? "fast"
            : "basic";
      ...
      s.enemies.push({
        ...
        color:
          type === "tank" ? "#f43f5e" : type === "fast" ? "#22d3ee" : "#a855f7",
        type,
      });
    }
  }, []);
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (
        ...
        (stateRef.current.gameOver || !stateRef.current.running)
      ) {
        startGame();
      }
    };
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (s.keys.has("ArrowLeft") || s.keys.has("KeyA"))
        p.x = Math.max(20, p.x - speed);
      if (s.keys.has("ArrowRight") || s.keys.has("KeyD"))
        p.x = Math.min(CANVAS_W - 20, p.x + speed);
      if (s.keys.has("ArrowUp") || s.keys.has("KeyW"))
        p.y = Math.max(20, p.y - speed);
      if (s.keys.has("ArrowDown") || s.keys.has("KeyS"))
        p.y = Math.min(CANVAS_H - 20, p.y + speed);
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
      <div className="relative">
        ...
        {gameState === "idle" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/75 rounded-lg">
            <h2
              className="font-arcade text-3xl text-red-400 mb-2"
              style={{ textShadow: "0 0 20px #f43f5e" }}
            >
              CYBER STRIKE
            </h2>
            <p className="font-rajdhani text-red-300 text-lg mb-6">
              Survive the cyber onslaught
            </p>
            <button
              type="button"
              onClick={startGame}
              className="font-arcade text-sm bg-red-700 hover:bg-red-600 text-white px-6 py-3 rounded border border-red-400 transition-all"
            >
              START GAME
            </button>
      ...
      <p className="font-rajdhani text-red-400/60 text-sm">
        WASD/Arrows to move • Auto-fire • Survive the waves!
      </p>
    </div>
  );
}
