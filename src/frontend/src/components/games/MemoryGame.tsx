    ...
    <div className="flex flex-col items-center gap-4 p-4 select-none">
      ...
      <div className="flex items-center gap-8 font-orbitron text-sm">
        <div className="flex items-center gap-2">
          <span className="text-foreground/50">MOVES</span>
          <span className="neon-text-cyan font-bold text-lg">{moves}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-foreground/50">PAIRS</span>
          <span className="neon-text-green font-bold text-lg">{matched}/8</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-foreground/50">TIME</span>
          <span className="neon-text-yellow font-bold text-lg">{time}s</span>
        </div>
      </div>
      ...
      {won && (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/80 z-50 gap-6">
          ...
          <div className="font-orbitron text-sm text-foreground/70 text-center">
            <div>
              Moves: <span className="neon-text-cyan">{moves}</span>
            </div>
            <div>
              Time: <span className="neon-text-yellow">{time}s</span>
            </div>
          </div>
          <button
            type="button"
            onClick={restart}
            className="px-8 py-4 font-orbitron font-bold text-sm border border-neon-green/50 bg-neon-green/10 text-neon-green rounded hover:bg-neon-green/20 transition-all"
      ...
      <button
        type="button"
        onClick={restart}
        className="px-4 py-2 font-orbitron text-xs border border-border/50 text-foreground/50 rounded hover:border-neon-pink/40 hover:text-neon-pink transition-all"
      >
        RESTART
      </button>
    </div>
