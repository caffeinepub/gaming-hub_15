...
export default function NeonRacersGame() {
  ...
  const getLaneX = useCallback(
    (lane: number) => LANE_W * lane + LANE_W / 2,
    [LANE_W],
  );
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: LANE_W is a module-level constant
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let lane = 1; lane < LANES; lane++) {
        ...
        ctx.shadowColor =
          lane === Math.floor(LANES / 2) ? "#facc15" : "transparent";
    ...
    const update = () => {
      ...
      else if (s.keys.has("ArrowDown") || s.keys.has("KeyS"))
        s.speed = Math.max(1, s.speed - 0.1);
      else s.speed = Math.max(3, s.speed - 0.02);

      // Lane change
      if (
        (s.keys.has("ArrowLeft") || s.keys.has("KeyA")) &&
        s.player.targetLane > 0
      ) {
      ...
      if (
        (s.keys.has("ArrowRight") || s.keys.has("KeyD")) &&
        s.player.targetLane < LANES - 1
      ) {
        if (s.player.laneX >= getLaneX(s.player.targetLane) - 1) {
      ...
      for (const ai of s.aiCars) {
        ...
        if (s.frameCount % 3 === 0) {
          s.particles.push({
            x: getLaneX(ai.lane) + (Math.random() - 0.5) * 10,
            y: ai.y - CAR_H / 2,
      ...
      if (s.frameCount % 2 === 0) {
        s.particles.push({
          x: s.player.laneX + (Math.random() - 0.5) * 8,
          y: s.player.y - CAR_H / 2,
          vx: (Math.random() - 0.5) * 1,
          vy: -s.speed * 0.3,
          life: 1,
          color: "#facc15",
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
      <div className="relative">
        ...
        {gameState === "idle" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/75 rounded-lg">
            <h2
              className="font-arcade text-3xl text-yellow-400 mb-2"
              style={{ textShadow: "0 0 20px #facc15" }}
            >
              NEON RACERS
            </h2>
            <p className="font-rajdhani text-yellow-300 text-lg mb-6">
              Race to the finish line!
            </p>
            <button
              type="button"
              onClick={startGame}
              className="font-arcade text-sm bg-yellow-700 hover:bg-yellow-600 text-white px-6 py-3 rounded border border-yellow-400 transition-all"
            >
              START RACE!
            </button>
        ...
        {gameState === "over" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
            ...
            <p className="font-arcade text-cyan-400 text-sm mb-1">
        ...
        {gameState === "won" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
            <h2
              className="font-arcade text-2xl text-yellow-400 mb-2"
              style={{ textShadow: "0 0 20px #facc15" }}
            >
              FINISH!
            </h2>
            <p className="font-arcade text-green-400 text-sm mb-1">
              SCORE: {displayScore}
            </p>
            <p className="font-arcade text-cyan-400 text-sm mb-6">
              OVERTAKES: {stateRef.current.overtakes}
            </p>
            <button
              type="button"
              onClick={startGame}
      ...
      <p className="font-rajdhani text-yellow-400/60 text-sm">
        ← → change lanes • ↑ accelerate • ↓ brake • Avoid other cars!
      </p>
    </div>
  );
}
