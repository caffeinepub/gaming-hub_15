...
export default function RuneQuestGame() {
  ...
  const [_selectedRune, setSelectedRune] = useState(0);
  ...
  const spawnLevel = useCallback((level: number) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const type: "goblin" | "orc" | "mage" =
        level > 2 && Math.random() < 0.25
          ? "mage"
          : level > 1 && Math.random() < 0.3
            ? "orc"
            : "goblin";
      s.enemies.push({
        id: eid2++,
        ...
        r: type === "orc" ? 20 : 14,
        color:
          type === "orc" ? "#4ade80" : type === "mage" ? "#a855f7" : "#f43f5e",
        type,
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.code === "Digit1") {
        stateRef.current.selectedRune = 0;
        setSelectedRune(0);
      }
      if (e.code === "Digit2") {
        stateRef.current.selectedRune = 1;
        setSelectedRune(1);
      }
      if (e.code === "Digit3") {
        stateRef.current.selectedRune = 2;
        setSelectedRune(2);
      }
      if (
        e.code === "Enter" &&
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (s.keys.has("ArrowLeft") || s.keys.has("KeyA"))
        p.x = Math.max(PLAYER_R, p.x - PLAYER_SPEED);
      if (s.keys.has("ArrowRight") || s.keys.has("KeyD"))
        p.x = Math.min(CANVAS_W - PLAYER_R, p.x + PLAYER_SPEED);
      if (s.keys.has("ArrowUp") || s.keys.has("KeyW"))
        p.y = Math.max(PLAYER_R, p.y - PLAYER_SPEED);
      if (s.keys.has("ArrowDown") || s.keys.has("KeyS"))
        p.y = Math.min(CANVAS_H - PLAYER_R, p.y + PLAYER_SPEED);
      ...
      s.runes = s.runes.filter(
        (r) =>
          r.life > 0 && r.x > 0 && r.x < CANVAS_W && r.y > 0 && r.y < CANVAS_H,
      );
            ...
            if (e.hp <= 0) {
              ...
              if (Math.random() < 0.3) {
                s.pickups.push({
                  x: e.x,
                  y: e.y,
                  type: Math.random() < 0.5 ? "mana" : "health",
                });
              }
            }
            break;
      ...
      if (p.iframes <= 0) {
        for (const r of s.runes) {
          if (
            r.color === "#ff6b6b" &&
            Math.hypot(r.x - p.x, r.y - p.y) < PLAYER_R + 6
          ) {
            p.hp--;
            p.iframes = 60;
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
      <div className="relative">
        ...
        {gameState === "idle" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/75 rounded-lg">
            <h2
              className="font-arcade text-3xl text-purple-400 mb-2"
              style={{ textShadow: "0 0 20px #a855f7" }}
            >
              RUNE QUEST
            </h2>
            <p className="font-rajdhani text-purple-300 text-lg mb-6">
              Cast runes to defeat your foes
            </p>
            <button
              type="button"
              onClick={startGame}
              className="font-arcade text-sm bg-purple-700 hover:bg-purple-600 text-white px-6 py-3 rounded border border-purple-400 transition-all"
            >
              START QUEST
            </button>
        ...
        {gameState === "over" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
            ...
            <p className="font-arcade text-purple-400 text-sm mb-1">
              SCORE: {displayScore}
            </p>
      ...
      <p className="font-rajdhani text-purple-400/60 text-sm">
        WASD to move • Click to cast runes • 1/2/3 switch rune
      </p>
    </div>
  );
}
