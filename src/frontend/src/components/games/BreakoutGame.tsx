  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      ...
      stateRef.current.paddle.x = Math.max(
        0,
        Math.min(W - PADDLE_W, mx - PADDLE_W / 2),
      );
    };
    ...
    <div className="flex flex-col items-center gap-4 p-4 select-none">
      <div className="flex items-center gap-8 font-orbitron text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-foreground/70 text-xs">← → / MOUSE</span>
        </div>
      </div>
      <div className="relative">
        <canvas
          ...
          width={W}
          height={H}
          className="border border-neon-pink/30 rounded"
        ...
        {(!started || gameOver || won) && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 rounded gap-4">
            {won && (
              <div className="font-arcade text-lg neon-text-green">
                YOU WIN!
              </div>
            )}
            {gameOver && (
            ...
            {!started && !gameOver && !won && (
              <div className="font-arcade text-base neon-text-cyan text-center leading-8">
                BREAKOUT
              </div>
            )}
            {(gameOver || won) && (
              <div className="font-orbitron text-sm text-foreground/70">
                Score: <span className="neon-text-green">{score}</span>
              </div>
            )}
            <div className="font-orbitron text-xs text-foreground/50 text-center">
              Move mouse or ← → to control paddle
            </div>
            <button
              type="button"
              onClick={startGame}
              className="mt-2 px-6 py-3 font-orbitron font-bold text-sm border border-neon-pink/50 bg-neon-pink/10 text-neon-pink rounded hover:bg-neon-pink/20 transition-all"
