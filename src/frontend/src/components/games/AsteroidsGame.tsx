...
export default function AsteroidsGame() {
  ...
  const draw = useCallback(() => {
    ...
    if (
      !s.gameOver &&
      (s.ship.invincible <= 0 || Math.floor(Date.now() / 100) % 2 === 0)
    ) {
      ctx.save();
  ...
  const loop = useCallback(() => {
    ...
    if (s.keys.fire && now - s.lastFire > 250) {
      s.bullets.push({
        pos: {
          x: ship.pos.x + Math.cos(ship.angle) * 22,
          y: ship.pos.y + Math.sin(ship.angle) * 22,
        },
        vel: {
          x: Math.cos(ship.angle) * 9 + ship.vel.x,
          y: Math.sin(ship.angle) * 9 + ship.vel.y,
        },
        life: 60,
      });
      s.lastFire = now;
    ...
    <div className="flex flex-col items-center gap-4 p-4 select-none">
      <div className="flex items-center gap-8 font-orbitron text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="neon-text-cyan font-bold text-lg">{score}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-foreground/50">WAVE</span>
          <span className="neon-text-green font-bold text-lg">{wave}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-foreground/50">LIVES</span>
          <span className="neon-text-pink font-bold text-lg">{lives}</span>
        </div>
      </div>
      <div className="relative">
        <canvas
          ...
          width={W}
          height={H}
          className="border border-neon-cyan/30 rounded"
        ...
        {(!started || gameOver) && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 rounded gap-4">
            {gameOver && (
              <div className="font-arcade text-lg neon-text-pink">
                GAME OVER
              </div>
            )}
            {!started && !gameOver && (
              <div className="font-arcade text-sm neon-text-cyan text-center leading-8">
                ASTEROIDS
              </div>
            )}
            {gameOver && (
              <div className="font-orbitron text-sm text-foreground/70">
                Score: <span className="neon-text-cyan">{score}</span>
              </div>
            )}
            <div className="font-orbitron text-xs text-foreground/50 text-center">
              ← → Rotate ↑ Thrust Space Shoot
            </div>
            <button
              type="button"
              onClick={startGame}
              className="mt-2 px-6 py-3 font-orbitron font-bold text-sm border border-neon-cyan/50 bg-neon-cyan/10 text-neon-cyan rounded hover:bg-neon-cyan/20 transition-all"
