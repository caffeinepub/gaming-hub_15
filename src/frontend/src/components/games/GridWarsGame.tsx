...
const UNIT_STATS: Record<
  UnitType,
  { hp: number; attack: number; range: number; color: string; symbol: string }
> = {
...
export default function GridWarsGame() {
  ...
  const getValidMoves = useCallback(
    (grid: Cell[][], row: number, col: number): [number, number][] => {
    ...
    (currentGrid: Cell[][]) => {
      ...
      const newGrid = currentGrid.map((row) =>
        row.map((cell) => ({
          ...cell,
          unit: cell.unit
            ? { ...cell.unit, moved: false, attacked: false }
            : null,
        })),
      );
    ...
    (row: number, col: number) => {
      ...
      if (selected) {
        ...
        if (phase === "move") {
          ...
          if (isValidMove && !selUnit.moved) {
            const newGrid = grid.map((r) =>
              r.map((c) => ({ ...c, unit: c.unit ? { ...c.unit } : null })),
            );
            ...
            setPhase("attack");
            addLog(
              `${selUnit.type} moved. Now select attack target or click elsewhere.`,
            );
            return;
          }
        ...
        if (phase === "attack") {
          ...
          const isValidAttack = attacks.some(
            ([r, c]) => r === row && c === col,
          );
          if (isValidAttack && !selUnit.attacked) {
            ...
            if (target.hp <= 0) {
              ...
              setScore((s) => s + 10);
              addLog(`Enemy ${target.type} destroyed! +10`);
  ...
  const validMoves = selected
    ? getValidMoves(grid, selected[0], selected[1])
    : [];
  const validAttacks = selected
    ? getValidAttacks(grid, selected[0], selected[1])
    : [];
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none p-4">
      <div className="flex items-center gap-6 mb-2">
        <span
          className="font-arcade text-cyan-400 text-sm"
          style={{ textShadow: "0 0 8px #22d3ee" }}
        >
          GRID WARS
        </span>
        <span className="font-arcade text-yellow-400 text-xs">
          SCORE: {score}
        </span>
        <span
          className={`font-arcade text-xs ${turn === "player" ? "text-cyan-400" : "text-red-400"}`}
        >
      ...
      <div className="flex gap-6">
        {/* Grid */}
        <div
          className="border-2 border-cyan-500/40 rounded-lg overflow-hidden"
            ...
            return (
              <div key={rowKey} className="flex">
                {row.map((cell, c) => {
                  ...
                  const isValidAttack =
                    phase === "attack" &&
                    validAttacks.some(([ar, ac]) => ar === r && ac === c);
                      ...
                      {unit && (
                        <>
                          ...
                          <span
                            ...
                            style={{
                              color:
                                unit.team === "player" ? "#22d3ee" : "#f43f5e",
                              fontSize: "6px",
                            }}
                          >
        ...
        <div className="flex flex-col gap-3 w-48">
              ...
              <div key={type} className="flex items-center gap-2 mb-1">
                ...
                <div>
                  ...
                  <p className="font-rajdhani text-xs text-gray-400">
                    HP:{stats.hp} ATK:{stats.attack} RNG:{stats.range}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="bg-slate-900/80 border border-cyan-500/30 rounded-lg p-3 flex-1">
            ...
            {log.map((msg, i) => (
              <p
                key={`log-${i}-${msg.slice(0, 8)}`}
                className="font-rajdhani text-xs text-gray-300 mb-1 opacity-"
      ...
      {selected && (
        <div className="flex gap-3">
          <span
            className={`font-arcade text-xs px-3 py-1 rounded border ${phase === "move" ? "border-green-400 text-green-400 bg-green-900/30" : "border-gray-600 text-gray-500"}`}
          >
            MOVE
          </span>
          <span
            className={`font-arcade text-xs px-3 py-1 rounded border ${phase === "attack" ? "border-red-400 text-red-400 bg-red-900/30" : "border-gray-600 text-gray-500"}`}
          >
            ATTACK
          </span>
        </div>
      ...
      {gameState !== "playing" && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/80 z-50">
          <div
            className="bg-slate-900 border-2 border-cyan-500 rounded-xl p-8 text-center"
            style={{ boxShadow: "0 0 40px rgba(34,211,238,0.4)" }}
          >
            <h2
              className="font-arcade text-2xl mb-2"
              style={{
                color: gameState === "won" ? "#4ade80" : "#f43f5e",
                textShadow: `0 0 20px ${gameState === "won" ? "#4ade80" : "#f43f5e"}`,
              }}
            >
              {gameState === "won" ? "VICTORY!" : "DEFEATED!"}
            </h2>
            <p className="font-arcade text-yellow-400 text-sm mb-6">
              SCORE: {score}
            </p>
            <button
              type="button"
              onClick={restart}
              className="font-arcade text-sm bg-cyan-700 hover:bg-cyan-600 text-white px-6 py-3 rounded border border-cyan-400 transition-all"
            >
              PLAY AGAIN
            </button>
          </div>
