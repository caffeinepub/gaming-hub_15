  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (s.keys.has("ArrowLeft") || s.keys.has("KeyA"))
        p.x = Math.max(FIELD_LEFT + PLAYER_R, p.x - PLAYER_SPEED);
      if (s.keys.has("ArrowRight") || s.keys.has("KeyD"))
        p.x = Math.min(CANVAS_W / 2 - PLAYER_R, p.x + PLAYER_SPEED);
      if (s.keys.has("ArrowUp") || s.keys.has("KeyW"))
        p.y = Math.max(FIELD_TOP + PLAYER_R, p.y - PLAYER_SPEED);
      if (s.keys.has("ArrowDown") || s.keys.has("KeyS"))
        p.y = Math.min(FIELD_BOTTOM - PLAYER_R, p.y + PLAYER_SPEED);

      // AI movement
      const ai = s.ai;
      const ball = s.ball;
      const aiDx = ball.x - ai.x;
      const aiDy = ball.y - ai.y;
      ...
      ai.x = Math.max(
        CANVAS_W / 2 + PLAYER_R,
        Math.min(FIELD_RIGHT - PLAYER_R, ai.x),
      );
      ai.y = Math.max(
        FIELD_TOP + PLAYER_R,
        Math.min(FIELD_BOTTOM - PLAYER_R, ai.y),
      );
      ...
      // Left goal (AI scores)
      if (
        ball.x - BALL_R < FIELD_LEFT - GOAL_W + 5 &&
        ball.y > goalY &&
        ball.y < goalY + GOAL_H
      ) {
        s.scoreAI++;
      ...
      if (
        ball.x + BALL_R > FIELD_RIGHT + GOAL_W - 5 &&
        ball.y > goalY &&
        ball.y < goalY + GOAL_H
      ) {
        s.scoreP++;
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
        ...
        {gameState === "over" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
            <h2
              ...
              style={{
                color: scoreP >= WIN_SCORE ? "#22d3ee" : "#f43f5e",
                textShadow: `0 0 20px ${scoreP >= WIN_SCORE ? "#22d3ee" : "#f43f5e"}`,
              }}
            >
            ...
            <button
              type="button"
              onClick={startGame}
              className="mt-4 font-arcade text-sm bg-yellow-700 hover:bg-yellow-600 text-white px-6 py-3 rounded border border-yellow-400 transition-all"
            >
              PLAY AGAIN
            </button>
          </div>
      ...
      <p className="font-rajdhani text-yellow-400/60 text-sm">
        WASD/Arrows to move • Run into ball to kick
      </p>
    </div>
  );
}
